...
import {
  CreateSaleBody,
  /*CreateSaleParams, CreateSaleType,*/ SaleSchema,
} from "../../../types/schemas/sale.js";
...
const ventasRoute: FastifyPluginAsync = async (fastify) => {
    ...
    handler: async (request, reply) => {
      ...
      if (!sale) {
        throw new UCUErrorBadRequest(
          `No se pudo crear la venta para el animal ${animal_id}`
        );
      }
